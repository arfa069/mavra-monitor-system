...
def test_build_profile_dir_defaults_to_project_root(monkeypatch):
    """Without explicit root, resolves relative to project root."""
    monkeypatch.delenv("PRICE_MONITOR_PROFILE_ROOT", raising=False)

    def _original_root():
        return Path(cp.__file__).resolve().parent.parent.parent.parent

    monkeypatch.setattr(cp, "_project_root", _original_root)
...
def test_build_profile_dir_uses_profile_root_env(monkeypatch, tmp_path):
    from app.core.crawler_paths import build_profile_dir

    monkeypatch.setenv("PRICE_MONITOR_PROFILE_ROOT", str(tmp_path))

    assert build_profile_dir("profile-a") == (
        tmp_path / "profiles" / "profile-a"
    )
