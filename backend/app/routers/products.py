    ...
    if config.cron_expression:
        ...
        if scheduler:
            ...
            try:
                ...
                logging.getLogger("app.routers.products").error("Failed to add job to scheduler: %s", exc)
...
async def delete_product_cron_config(
    ...
    if scheduler:
        ...
        try:
            ...
            logging.getLogger("app.routers.products").error("Failed to remove job from scheduler: %s", exc)
            raise HTTPException(status_code=400, detail=f"Scheduler error: {str(exc)}")
...
async def update_product_cron_config(
    ...
    if scheduler:
        ...
        try:
            ...
            logging.getLogger("app.routers.products").error("Failed to sync scheduler: %s", exc)
            raise HTTPException(status_code=400, detail=f"Scheduler error: {str(exc)}")
