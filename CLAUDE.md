...
This project is indexed by GitNexus as **mavra-monitor-system** (10483 symbols, 19235 relationships, 300 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
...
3. 运行 `./scripts/generate_dart_client.ps1`。
4. 前端通过 `frontend/lib/core/api/generated/` 适配；repository 可以做数据归一化、轮询、缓存失效或 UI 映射，但不能绕过 generated client。
5. 运行 `uv run --project backend --extra dev python scripts/check_api_contract.py` 和 `uv run --project backend --extra dev python scripts/check_dart_api_usage.py`。
