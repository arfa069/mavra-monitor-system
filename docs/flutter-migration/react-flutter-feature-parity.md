...
| Schedule | `frontend/src/features/schedule/ScheduleConfigPage.tsx` | Task 16 in progress: product/job schedule sections, cron generator, rule save, retention days, and Feishu webhook settings save are available | Remaining follow-up: richer cron presets and inline update/delete of existing product cron rows |
| Events | `frontend/src/features/events/EventCenterPage.tsx` | Task 16 in progress: kind, type, category, severity, source, keyword, date range filters, pagination, detail dialog, and SSE merge are available | Remaining follow-up: denser table styling and saved filter presets |
| Admin | `frontend/src/features/admin/` | Task 16 in progress: user filters, role filter, audit action/actor filters, user/audit pagination, create/edit/enable/disable/delete user intents, permission matrix, and audit list are available | Remaining follow-up: fully split route-specific Users and Audit Logs layouts beyond the shared Admin shell |
| Blog | `frontend/src/features/blog/BlogAdminPage.tsx` | Task 16 in progress: search/status filters, taxonomy display, category/tag editing, cover upload, publish time, SEO title/description, and editor form are available | Remaining follow-up: richer category/tag picker widgets and dedicated modal shell polish |
| Smart Home | `frontend/src/features/smart-home/SmartHomePage.tsx` | Task 16 in progress: config edit/test, entity filters, entity-level turn on/off confirmation, manual service request form, read-only control gating, and realtime entity updates are available | Remaining follow-up: richer service picker and entity detail panels |
