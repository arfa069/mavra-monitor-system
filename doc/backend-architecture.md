...
- `app.workers.crawler` — worker CLI：`python -m app.workers.crawler --kind job --platform boss`，可按 kind/platform 过滤，也可用 `--once` 做单轮验证。
...
- `GET /crawl/workers` — 查看 worker_id、kind、platform、hostname、pid、status、heartbeat。
...
- `GET /smart-home/config` / `PUT /smart-home/config` / `POST /smart-home/config/test` 负责配置查看、更新和连通性测试。
- `GET /smart-home/entities` 返回当前可用实体列表；`POST /smart-home/services/call` 通过请求体中的 `entity_id`、`service` 和 `service_data` 发送控制指令。
- `GET /smart-home/entities/stream` 通过 SSE 将 Home Assistant state 变更广播到前端页面。
