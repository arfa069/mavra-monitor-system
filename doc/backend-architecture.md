...
│   │   ├── jd_opencli.py       # JD OpenCLI 子进程价格提取
│   │   ├── taobao_opencli.py   # 淘宝 OpenCLI 子进程价格提取
│   │   ├── firecrawl_product.py # Firecrawl Cloud API 商品爬取兜底
...
## 8. 平台适配器架构（重构后：OpenCLI/Firecrawl 替代 Playwright 商品爬取）

### 8.0 架构变更摘要

商品爬取在 2026-06 重构中从 Playwright/CDP 浏览器适配器迁移到 **OpenCLI 子进程 + Firecrawl Cloud API**。Job 爬取保持不变（curl_cffi HTTP）。

```
backend/app/platforms/base.py              — BasePlatformAdapter (ABC，CDP fallback 保留)
backend/app/platforms/jd_opencli.py        — JD OpenCLI 子进程提取价格
backend/app/platforms/taobao_opencli.py    — 淘宝 OpenCLI 子进程提取价格
backend/app/platforms/firecrawl_product.py — Firecrawl Cloud API 兜底
...
**商品爬取主路径**：`domains/crawling/service.py:crawl_one_opencli()` → 按 platform 选择 `jd_opencli.crawl_jd_via_opencli()` 或 `taobao_opencli.crawl_taobao_via_opencli()`。兜底路径：`crawl_one_firecrawl()` → `firecrawl_product.crawl_product_via_firecrawl()`。

`taobao.py`、`jd.py`、`amazon.py`、`boss.py` 的 Playwright/CDP 适配器已删除。`base.py` 保留 `BasePlatformAdapter` ABC 供 CDP fallback 场景使用，但不再作为商品爬取主路径。

### 8.1 BasePlatformAdapter（platforms/base.py — CDP fallback only）
...
| 适配器                             | 提取策略                                                                         |
| ---------------------------------- | -------------------------------------------------------------------------------- |
| TaobaoOpencli（crawl_one_opencli） | 子进程调 opencli 提取价格，零浏览器开销                                          |
| JdOpencli（crawl_one_opencli）     | 同上                                                                             |
| Firecrawl（crawl_one_firecrawl）   | Firecrawl Cloud API 兜底爬取                                                     |
...
- `crawl_one_opencli()` 或 `crawl_one_firecrawl()` 被 worker 进程调用，无 Celery 依赖
