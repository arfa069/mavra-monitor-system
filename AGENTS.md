...
This project is indexed by GitNexus as **price-monitor** (6359 symbols, 12034 relationships, 241 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
...
- Profile 是并发资源：一个 profile 可保存多个平台登录态，但同一时刻只能被一个爬取任务占用；需要并发时使用多个 profile key，而不是让多个任务同时打开同一目录。
