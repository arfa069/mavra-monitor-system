...
const cnDayNames = Object.keys(CN_DAY_MAP)
...
const MATCHERS: Matcher[] = [
  ...
  {
    pattern:
      /^(每天|每日)(早上|上午|下午|晚上|傍晚|凌晨)?(\d{1,2})点(半|(\d{1,2})分)?$/,
    build: (m) => {
      const t = cnParseTime(m[2], m[3], m[4], m[5]);
      if (!t) return null;
      return {
        cron: `${t.minute} ${t.hour} * * *`,
        description: `每天 ${formatTime(t.hour, t.minute)}`,
      };
    },
  },
  ...
  {
    pattern:
      /^(工作日)(早上|上午|下午|晚上|傍晚|凌晨)?(\d{1,2})点(半|(\d{1,2})分)?$/,
    build: (m) => {
      const t = cnParseTime(m[2], m[3], m[4], m[5]);
      if (!t) return null;
      return {
        cron: `${t.minute} ${t.hour} * * 1-5`,
        description: `工作日 ${formatTime(t.hour, t.minute)}`,
      };
    },
  },
  {
    pattern:
      /^(周末)(早上|上午|下午|晚上|傍晚|凌晨)?(\d{1,2})点(半|(\d{1,2})分)?$/,
    build: (m) => {
      const t = cnParseTime(m[2], m[3], m[4], m[5]);
      if (!t) return null;
      return {
        cron: `${t.minute} ${t.hour} * * 0,6`,
        description: `周末 ${formatTime(t.hour, t.minute)}`,
      };
    },
  },
  ...
  {
    pattern: new RegExp(
      `^每(${cnDayNames})(早上|上午|下午|晚上|傍晚|凌晨)?(\\d{1,2})点(半|(\\d{1,2})分)?$`,
    ),
    build: (m) => {
      ...
      return {
        ...
        description: `每${m[1]} ${formatTime(t.hour, t.minute)}`,
      };
    },
  },
  {
  ...
  {
    pattern:
      /^每月(\d{1,2})号(早上|上午|下午|晚上|傍晚|凌晨)?(\d{1,2})点(半|(\d{1,2})分)?$/,
    build: (m) => {
      ...
      return {
        ...
        description: `每月${day}号 ${formatTime(t.hour, t.minute)}`,
      };
    },
  },
  {
  ...
  },
  {
    ...
    build: () => ({
      cron: "0 * * * *",
      description: "At minute 0, every hour",
    }),
  },
  {
  ...
  },
  {
    ...
    build: () => ({
      cron: "0 0 * * 1-5",
      description: "At midnight, Monday through Friday",
    }),
  },
  {
    pattern: /^weekends?$/,
    build: () => ({
      cron: "0 0 * * 0,6",
      description: "At midnight, Saturday and Sunday",
    }),
  },
  ...
  {
    pattern:
      /^every\s+(monday|tuesday|wednesday|thursday|friday|saturday|sun|mon|tue|wed|thu|fri|sat|sunday)\s+at\s+(\d{1,2})(?::(\d{2}))?\s*(am|pm)?$/i,
    build: (m) => {
      ...
      if (!t) return null;
      const dayLabel =
        Object.entries(DAY_MAP).find(([, v]) => v === dayNum)?.[0] ?? "";
      return {
        cron: `${t.minute} ${t.hour} * * ${dayNum}`,
  ...
  {
    pattern:
      /^(weekdays?|every\s+weekday)\s+at\s+(\d{1,2})(?::(\d{2}))?\s*(am|pm)?$/i,
    build: (m) => {
      const t = parseTime(m[2], m[3], m[4]);
      if (!t) return null;
      return {
  ...
  {
    pattern:
      /^(weekends?|every\s+weekend)\s+at\s+(\d{1,2})(?::(\d{2}))?\s*(am|pm)?$/i,
    build: (m) => {
      const t = parseTime(m[2], m[3], m[4]);
      if (!t) return null;
      return {
  ...
  {
    pattern:
      /^(monthly|every\s+month)\s+on\s+the\s+(\d{1,2})(?:st|nd|rd|th)?(?:\s+at\s+(\d{1,2})(?::(\d{2}))?\s*(am|pm)?)?$/i,
    build: (m) => {
