...
export default function BatchImportModal({
  ...
  const handleParse = () => {
    ...
    if (filtered.length < deduped.length) {
      message.info(
        `Skipped ${deduped.length - filtered.length} duplicate URL(s)`,
      );
    }
  ...
  const handleConfirm = () => {
    if (items.some((item) => !item.platform)) {
      message.error(
        "Some URLs have unrecognized platform, please select manually",
      );
      return;
    }
    ...
    >
      ...
      {step === 0 ? (
        <div>
          ...
          />
          <div
            style={{ color: "var(--color-muted)", fontSize: 12, marginTop: 8 }}
          >
