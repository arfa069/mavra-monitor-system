...
test.describe("Settings and Schedule E2E", () => {
  test("renders settings form, handles PATCH update and persists motion settings", async ({
    page,
    api,
  }) => {
    ...
    await expect(
      page.locator('input[placeholder*="open.feishu.cn"]'),
    ).toHaveValue("https://open.feishu.cn/open-apis/bot/v2/hook/old");
    await expect(page.locator("#data_retention_days")).toHaveValue("30");

    // Modify settings and submit
    await page.fill(
      'input[placeholder*="open.feishu.cn"]',
      "https://open.feishu.cn/open-apis/bot/v2/hook/new",
    );
  ...
  test("validates job schedule config cron expression", async ({
    page,
    api,
  }) => {
    ...
    const saveButton = row.locator(
      'button:has-text("Save"), button:has(.anticon-save)',
    );
    ...
    await expect(
      page.locator(".ant-message-notice", {
        hasText: "Invalid cron expression",
      }),
    ).toBeVisible();
    ...
    await expect(
      page.locator(".ant-message-notice", { hasText: "Saved" }),
    ).toBeVisible();
  ...
  test("disables schedule controls for read-only user", async ({
    page,
    api,
  }) => {
