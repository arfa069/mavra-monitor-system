...
test.describe("Authentication E2E", () => {
  test("redirects unauthenticated users to login page and back after login", async ({ page, api }) => {
    ...
    await page.waitForURL((url) => url.pathname === "/settings" || url.pathname === "/dashboard");
  ...
  test("displays validation errors on empty submission", async ({ page, api }) => {
    api.use("GET", "/api/v1/auth/me", () => ({
      status: 401,
      body: { detail: "Not authenticated" }
    }));
    ...
    await expect(page.locator(".ant-form-item-explain-error", { hasText: "Please enter username or email" })).toBeVisible();
    await expect(page.locator(".ant-form-item-explain-error", { hasText: "Please enter password" })).toBeVisible();
  });

  test("handles login failure and resets password input", async ({ page, api }) => {
    api.use("GET", "/api/v1/auth/me", () => ({
      status: 401,
      body: { detail: "Not authenticated" }
    }));
    ...
    await expect(page.locator(".ant-message-notice", { hasText: "Invalid username or password" })).toBeVisible();
  ...
  test("does not store token or user details in localStorage", async ({ page, api }) => {
    api.use("GET", "/api/v1/auth/me", () => ({
      status: 401,
      body: { detail: "Not authenticated" }
    }));
