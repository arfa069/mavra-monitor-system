...
export const test = base.extend<Fixtures>({
  api: async ({ page }, use) => {
    ...
    api.use("GET", "/api/v1/dashboard/kpi", () => ({
      body: {
        user: { total_products: 5, price_drops_today: 1, new_jobs_today: 2, match_count: 1, crawl_count_today: 10 },
        system: { total_users: 2, total_crawls: 50, success_rate: 99.0, active_alerts: 1, disk_usage: 30.0, memory_usage: 45.0 }
      }
    }));
    api.use("GET", "/api/v1/dashboard/trends", () => ({
      body: {
        labels: ["Mon", "Tue", "Wed"],
        datasets: [{ label: "Price Changes", data: [{ label: "Mon", value: 1 }] }]
      }
    }));
    ...
    api.use("GET", "/api/v1/smart-home/entities/stream", () => ({ status: 200, body: {} }));
    ...
    api.use("GET", "/api/v1/scheduler/status", () => ({ body: { is_running: true } }));
    api.use("GET", "/api/v1/products/cron-configs", () => ({ body: [] }));
    api.use("GET", "/api/v1/products/cron-schedules", () => ({ body: { platforms: {} } }));
    api.use("GET", "/api/v1/jobs/scheduler/job-configs", () => ({ body: { configs: [] } }));
