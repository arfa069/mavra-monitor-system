...
test.describe("API Mock Firewall", () => {
  ...
  test("records UNMOCKED and responds 501 for unregistered api calls", async ({ page }) => {
    const api = new ApiMock();
    api.use("GET", "/api/v1/auth/me", () => ({ status: 401, body: { detail: "unauthorized" } }));
  ...
  test("records BLOCKED and aborts dangerous endpoints", async ({ page }) => {
    ...
    expect(api.getViolations()).toContain("BLOCKED POST /api/v1/crawl/crawl-now");
  });
