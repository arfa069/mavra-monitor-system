...
test.describe("Products and Jobs Feature E2E", () => {
  test("renders product list, filters, and supports adding a product", async ({
    page,
    api,
  }) => {
    ...
    api.use("GET", "/api/v1/products", (request) => {
      ...
      const keyword =
        url.searchParams.get("keyword") || url.searchParams.get("search") || "";
    ...
    await expect(
      page.locator("text=Synthetic Marketplace Item A"),
    ).toBeVisible();
    await expect(
      page.locator("text=Synthetic Marketplace Item B"),
    ).toBeVisible();

    // Apply search filter
    const searchInput = page
      .locator('input[placeholder*="Search"], input.ant-input')
      .first();
    ...
    // Click Add Product
    await page.click(
      'button:has-text("Add Product"), button:has(.anticon-plus)',
    );
    // Modal appears
  ...
  test("renders jobs dashboard, detail drawer, profiles, and hides crawl now for readonly", async ({
    page,
    api,
  }) => {
    ...
    await expect(
      page.locator("text=Boss Software Engineer Monitor"),
    ).toBeVisible();
    ...
    const detailButton = page
      .locator(
        'button:has(.anticon-history), button:has-text("History"), button:has-text("Details")',
      )
      .first();
    ...
    await expect(
      page
        .locator(
          ".ant-tabs-tabpane-active .ant-table-cell >> text=boss_chrome_profile",
        )
        .first(),
    ).toBeVisible();
    await expect(
      page
        .locator(
          ".ant-tabs-tabpane-active .ant-table-cell >> text=liepin_chrome_profile",
        )
        .first(),
    ).toBeVisible();
    await expect(
      page
        .locator(".ant-tabs-tabpane-active .ant-table-cell >> text=leased")
        .first(),
    ).toBeVisible();
    ...
    await expect(
      page.locator('button:has-text("Crawl Now"), button:has(.anticon-rocket)'),
    ).toHaveCount(0);
    await expect(page.locator('button:has-text("Test Profile")')).toHaveCount(
      0,
    );
  });
});
