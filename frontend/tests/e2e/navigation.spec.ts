...
test.describe("Navigation and Permission E2E", () => {
  test.beforeEach(async ({ api }) => {
    api.use("GET", "/api/v1/products", () => ({ body: { items: [], total: 0 } }));
    api.use("GET", "/api/v1/jobs/configs", () => ({ body: [] }));
    api.use("GET", "/api/v1/jobs/profiles", () => ({ body: [] }));
    api.use("GET", "/api/v1/jobs/scheduler/status", () => ({ body: { is_running: true } }));
    api.use("GET", "/api/v1/smart-home/config", () => ({ body: { url: "", token_configured: false } }));
    api.use("GET", "/api/v1/smart-home/entities", () => ({ body: [] }));
    api.use("GET", "/api/v1/admin/users", () => ({ body: { items: [], total: 0 } }));
    ...
    api.use("GET", "/api/v1/admin/audit-logs", () => ({ body: { items: [], total: 0 } }));
    api.use("GET", "/api/v1/events", () => ({ body: { items: [], total: 0 } }));
    api.use("GET", "/api/v1/events/stream", () => ({ status: 200, body: {} }));
    api.use("GET", "/api/v1/smart-home/stream", () => ({ status: 200, body: {} }));
  });
  ...
  test("navigates to main pages via sidebar", async ({ page }) => {
    ...
    // Click Event Center
    await page.click('a[href="/events"], .ant-menu-item:has-text("Event Center")');
    await page.waitForURL("**/events");
    await expect(page.locator("[data-page-transition]")).toBeVisible();

    // Click Job Management
    await page.click('a[href="/jobs"], .ant-menu-item:has-text("Job Management")');
    await page.waitForURL("**/jobs");
    await expect(page.locator("[data-page-transition]")).toBeVisible();

    // Click Product Management
    await page.click('a[href="/products"], .ant-menu-item:has-text("Product Management")');
    await page.waitForURL("**/products");
    await expect(page.locator("[data-page-transition]")).toBeVisible();

    // Click Schedule Config
    await page.click('a[href="/schedule"], .ant-menu-item:has-text("Schedule Config")');
    await page.waitForURL("**/schedule");
    await expect(page.locator("[data-page-transition]")).toBeVisible();

    // Click Smart Home
    await page.click('a[href="/smart-home"], .ant-menu-item:has-text("Smart Home")');
    await page.waitForURL("**/smart-home");
    await expect(page.locator("[data-page-transition]")).toBeVisible();
  });

  test("redirects user without user:read from /admin/users to /dashboard", async ({ page, api }) => {
  ...
  test("persists motion settings in localStorage and handles settings change", async ({ page, api }) => {
    ...
    if (await slowRadio.count() > 0) {
      ...
      const speed = await page.evaluate(() => localStorage.getItem("mavra-monitor-system-motion-speed"));
